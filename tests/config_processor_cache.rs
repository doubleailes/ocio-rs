...
fn dv(cfg: &Config, ctx: Option<&Context>, view: &str) -> Processor {
    let c = ctx.cloned().unwrap_or_else(|| cfg.current_context().clone());
    cfg.get_display_view_processor_with_context(&c, "cs1", "disp1", view, TransformDirection::Forward)
        .unwrap()
}
...
fn config_context_variables_typical_use_cases() {
    let _lock = env_lock();
    let dir = data_file("");
    for v in ["FILE", "ENV", "SHOW", "SHOT", "TRANSFORM_DIR", "PATH_1", "PATH_2", "CCPREFIX", "CCNUM"] {
    ...
    // Case 1 - No context variables used in the config.
    {
        ...
        let cfg = load(&config);
        assert!(same_instance(&cs(&cfg, None, "cs1", "cs2"), &cs(&cfg, None, "cs1", "cs2")));
        assert!(same_instance(&cs(&cfg, None, "cs1", "cs3"), &cs(&cfg, None, "cs1", "cs3")));
        assert!(same_instance(&dv(&cfg, None, "view1"), &dv(&cfg, None, "view1")));
        assert!(same_instance(&dv(&cfg, None, "view2"), &dv(&cfg, None, "view2")));

        let mut ctx = cfg.current_context().clone();
        for _ in 0..2 {
            let c = Some(&ctx);
            assert!(same_instance(&cs(&cfg, c, "cs1", "cs2"), &cs(&cfg, c, "cs1", "cs2")));
            assert!(same_instance(&cs(&cfg, c, "cs1", "cs3"), &cs(&cfg, c, "cs1", "cs3")));
            assert!(same_instance(&cs(&cfg, c, "cs1", "cs2"), &cs(&cfg, None, "cs1", "cs2")));
            assert!(same_instance(&dv(&cfg, c, "view1"), &dv(&cfg, c, "view1")));
            assert!(same_instance(&dv(&cfg, c, "view2"), &dv(&cfg, c, "view2")));
            assert!(same_instance(&dv(&cfg, c, "view1"), &dv(&cfg, None, "view1")));
    ...
    );
    {
        ...
        assert!(same_instance(&cs(&cfg, None, "cs1", "cs2"), &cs(&cfg, Some(&ctx), "cs1", "cs2")));
        assert!(same_instance(&cs(&cfg, None, "cs1", "cs2"), &cs(&cfg, None, "cs1", "cs3")));
    }
    {
    ...
    for disable in [false, true] {
        let _fb = EnvGuard::set(OCIO_DISABLE_CACHE_FALLBACK, if disable { Some("1") } else { None });
        let cfg = load(&case4);
        let mut ctx = cfg.current_context().clone();
    ...
    );
    {
        ...
        assert!(same_instance(&cs(&cfg, Some(&ctx1), "cs1", "cs2"), &cs(&cfg, Some(&ctx2), "cs1", "cs2")));
        {
            let _fb = EnvGuard::set(OCIO_DISABLE_CACHE_FALLBACK, Some("1"));
            let cfg = load(&case5);
            assert!(!same_instance(&cs(&cfg, Some(&ctx1), "cs1", "cs2"), &cs(&cfg, Some(&ctx2), "cs1", "cs2")));
    ...
    // Case 7 - Context variables in the FileTransform's CCCID.
    {
        ...
        for n in ["01", "02", "03"] {
            ...
            procs.push(cfg.get_processor_with_context(&ctx, &t, TransformDirection::Forward).unwrap());
        }
