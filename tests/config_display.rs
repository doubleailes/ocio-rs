...
fn shared_views_basic() {
    ...
    config.add_shared_view("shared1", "", "colorspace", "", "", "").unwrap();
    assert_err!(config.serialize(), "Only version 2 (or higher) can have shared views");
    ...
    config.add_shared_view("shared1", "", "colorspace1", "", "", "").unwrap();
    assert_err!(
        config.validate(),
    ...
    config.add_color_space(&cs).unwrap();
    config.add_shared_view("shared2", "", "colorspace2", "look1", "", "").unwrap();
    assert_err!(config.validate(), "refers to a look, 'look1', which is not defined.");
    ...
    config.add_color_space(&cs).unwrap();
    config
        .add_shared_view("shared3", "viewTransform1", "colorspace3", "", "", "shared view description")
        .unwrap();
    assert_err!(
        config.validate(),
    ...
    config.add_shared_view("shared4", "", "colorspace1", "", "rule1", "").unwrap();
    assert_err!(config.validate(), "viewing rule, 'rule1', which is not defined");
    ...
    config.add_display_view("sRGB", "view1", "colorspace1", "").unwrap();
    config.validate().unwrap();
    ...
    assert_eq!(config.view_by_type(ViewType::DisplayDefined, "sRGB", 0), "Raw");
    assert_eq!(config.view_by_type(ViewType::DisplayDefined, "sRGB", 1), "view1");
    ...
    assert_eq!(config.display_view_color_space_name("sRGB", "view1"), "colorspace1");
    assert_eq!(config.display_view_color_space_name("sRGB", "shared2"), "colorspace2");
    assert_eq!(config.display_view_transform_name("sRGB", "shared3"), "viewTransform1");
    assert_eq!(config.display_view_looks("sRGB", "shared2"), "look1");
    assert_eq!(config.display_view_rule("sRGB", "shared4"), "rule1");
    assert_eq!(config.display_view_description("sRGB", "shared3"), "shared view description");

    // An empty display name may be used to access shared views.
    assert_eq!(config.display_view_color_space_name("", "shared1"), "colorspace1");
    assert_eq!(config.display_view_color_space_name("", "shared2"), "colorspace2");
    assert_eq!(config.display_view_looks("", "shared2"), "look1");
    assert_eq!(config.display_view_transform_name("", "shared3"), "viewTransform1");
    assert_eq!(config.display_view_color_space_name("", "shared3"), "colorspace3");
    assert_eq!(config.display_view_rule("", "shared4"), "rule1");
    assert_eq!(config.display_view_description("", "shared5"), "Sample description");
    ...
    assert_eq!(back.display_view_transform_name("", "shared3"), "viewTransform1");
    assert_eq!(back.display_view_color_space_name("", "shared3"), "colorspace3");
    assert_eq!(back.display_view_rule("", "shared4"), "rule1");
    assert_eq!(back.display_view_description("", "shared5"), "Sample description");
    ...
    config.add_display_view("sRGB", "shared1", "colorspace1", "").unwrap();
    config.validate().unwrap();
    assert_err!(
    ...
    let back = Config::create_from_str(&s).unwrap();
    assert_eq!(back.display_view_color_space_name("", "shared3"), OCIO_VIEW_USE_DISPLAY_NAME);
...
fn config_compare_displays() {
...

    {
        ...
        assert_eq!(config1.num_views_by_type(ViewType::DisplayDefined, "sRGB"), 2);
        assert_eq!(config1.view_by_type(ViewType::DisplayDefined, "sRGB", 0), "Raw");
        assert_eq!(config1.view_by_type(ViewType::DisplayDefined, "sRGB", 1), "view");
    ...
    }
    {
        ...
        assert_eq!(config1.num_views_by_type(ViewType::DisplayDefined, "Raw"), 1);
        assert_eq!(config1.view_by_type(ViewType::DisplayDefined, "Raw", 0), "Raw");
        assert!(!config1.is_view_shared("Raw", "Raw"));

        assert_eq!(config2.num_views_by_type(ViewType::DisplayDefined, "Raw"), 1);
        assert_eq!(config2.view_by_type(ViewType::DisplayDefined, "Raw", 0), "Raw");
    ...
    }
    {
        ...
        assert!(Config::are_views_equal(&config1, &config2, "sRGB", "sview1"));
    }
    {
    ...
    }
    {
        ...
        assert_eq!(cfg1.view_by_type(ViewType::DisplayDefined, "sRGB", 0), "view");
        assert!(!cfg1.has_view("sRGB", "Raw"));
        assert!(!Config::are_views_equal(&config1, &cfg1, "sRGB", "Raw"));
    }
    {
...
fn check_virtual_view(cfg: &Config, name: &str, vt: &str, cs: &str, looks: &str, rule: &str, desc: &str) {
...
fn config_compare_virtual_displays() {
...

    {
        ...
        let v1 = config1.virtual_display_view(ViewType::Shared, 0).to_string();
        assert_eq!(v1, "Film");
        check_virtual_view(&config1, &v1, "display_vt", "<USE_DISPLAY_NAME>", "look1", "Linear", "Test view");

        assert_eq!(config2.virtual_display_num_views(ViewType::DisplayDefined), 2);
        let v2 = config2.virtual_display_view(ViewType::DisplayDefined, 1).to_string();
        assert_eq!(v2, "Film");
        check_virtual_view(&config2, &v2, "display_vt", "<USE_DISPLAY_NAME>", "look1", "Linear", "Test view");

        assert!(Config::are_virtual_views_equal(&config1, &config2, &v1));
    }
    {
        assert_eq!(config1.virtual_display_num_views(ViewType::DisplayDefined), 1);
        let v1 = config1.virtual_display_view(ViewType::DisplayDefined, 0).to_string();
        assert_eq!(v1, "Raw");
        check_virtual_view(&config1, &v1, "", "raw", "", "", "");
        let v2 = config2.virtual_display_view(ViewType::DisplayDefined, 0).to_string();
        assert_eq!(v2, "Raw");
        check_virtual_view(&config2, &v2, "", "raw", "", "", "");
        assert!(Config::are_virtual_views_equal(&config1, &config2, &v1));
    }
    {
        let v1 = config1.virtual_display_view(ViewType::Shared, 1).to_string();
        assert_eq!(v1, "view");
        check_virtual_view(&config1, &v1, "display_vt", "display_cs", "", "", "");
        assert_eq!(config2.virtual_display_num_views(ViewType::Shared), 1);
        let v2 = config2.virtual_display_view(ViewType::Shared, 0).to_string();
        assert_eq!(v2, "view");
